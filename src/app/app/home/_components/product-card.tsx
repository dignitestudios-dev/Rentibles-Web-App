...
const ProductCard: React.FC<{ product?: Product }> = ({ product }) => {
  ...
  const review = product?.productReview ?? 4.5;
    ...
    <div className="bg-card rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 h-96 hover:-translate-y-2">
      <div className="p-4 h-full flex flex-col">
        <div className="relative rounded-2xl overflow-hidden bg-gray-100">
          <img src={cover} alt={name} className="w-full h-56 object-cover" />

          <div className="absolute top-3 left-3 bg-gray-800 text-white rounded-md px-2 py-1 flex items-center gap-2 text-sm">
            <Star className="w-4 h-4 text-yellow-400" />
            <span className="font-semibold">{review}</span>
          </div>

          <button className="absolute top-3 right-3 bg-white rounded-md p-2 shadow flex items-center justify-center">
            <Heart
